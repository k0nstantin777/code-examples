...
use App\Services\TelegramBot\Flows\ExchangeRequest\States\AwaitEnterPaymentTransactionIDState;
use App\Services\TelegramBot\Flows\ExchangeRequest\States\CancelledExchangeRequestState;
use App\Services\TelegramBot\Flows\ExchangeRequest\States\PaidExchangeRequestState;
...
class AwaitCustomerActionForExchangeRequestStateFlowTest extends TestCase
{
    use TelegramBotApiMock;
    use JsonRpcClientMock;
    use RefreshDatabase;

    public function testCancelExchangeRequestBtnPressed(): void
    {
        Queue::fake();
        ...
        $button1 = $buttonService->makeCancelExchangeRequestButton('c4bed09a-3f3c-495a-b920-b328d574479f');
    ...
    public function testPayExchangeRequestBtnPressedWhenTransactionIdNotPresent(): void
    {
        $this->initJsonRpcClientMock();
        $this->initTelegramBotApiMock();
        ...
        $response->assertStatus(200);
        $expectedState = app(AwaitEnterPaymentTransactionIDState::class);
        $expectedState->setExchangeRequest($exchangeRequest);
    ...
    public function testPayExchangeRequestBtnPressedWithTransactionId(): void
    {
        $this->initJsonRpcClientMock();
        $this->initTelegramBotApiMock();
        ...
        $response->assertStatus(200);
        $expectedState = app(PaidExchangeRequestState::class);
        $expectedState->setExchangeRequest($exchangeRequest);

        $this->assertEquals([
            $expectedState,
            'The application has been successfully marked as paid, we will check the payment and inform you',
        ], [
            $exchangeRequest->getState(),
            $messages[0]->text,
        ]);
    ...
    public function testPayExchangeRequestBtnPressedWhenPaymentAddressNotPresent(): void
    {
        $this->initJsonRpcClientMock();
        $this->initTelegramBotApiMock();
